...
## Requirements
- Node.js 20+
- macOS, Linux, or Windows with read/write access to the destination drive
...
Example JSONL entry:
```json
{"action":"copy","source":"C:\\media\\IMG_0001.JPG","destination":"D:\\sorted\\photo\\2023\\11\\IMG_0001.JPG","status":"success","sourceHash":"abc123...","destHash":"abc123...","startTime":"2025-01-01T12:00:00.000Z","endTime":"2025-01-01T12:00:01.250Z","fileSize":3456789}
```
...
| --- | --- |
| CLI exits with “source path … outside of the source root” | Ensure `--source-root` matches the directory that contained your files when the plan was generated. Use absolute paths. |
| `MediaInfoModule.wasm` not found | Run `npm install` at the repo root so the postinstall script copies the WASM into `public/mediainfo`. |
| Verification failures (`dest hash != source hash`) | Destination storage may be unreliable. Re-run with `--resume --verify`; the CLI will stop on the first mismatch. |
| Permission denied / EPERM | Run the CLI with sufficient permissions for both source and destination drives (on Windows, elevated PowerShell sometimes required). |
